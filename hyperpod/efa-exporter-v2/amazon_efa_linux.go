...
// Global pod-device mapping, refreshed periodically
var (
...
func fetchPodResources(logger *slog.Logger) map[string]podInfo {
	...
	if v := os.Getenv("KUBELET_SOCKET"); v != "" {
		socketPath = v
	}

	if _, err := os.Stat(socketPath); err != nil {
		logger.Debug("kubelet pod-resources socket not found", "path", socketPath)
		return result
	}
	...
	if err != nil {
		logger.Error("failed to dial kubelet pod-resources", "err", err)
		return result
	}
	defer conn.Close()
	...
	if err != nil {
		logger.Error("failed to list pod resources", "err", err)
		return result
	}
	...
	for _, pod := range resp.GetPodResources() {
		for _, container := range pod.GetContainers() {
			for _, dev := range container.GetDevices() {
				if dev.GetResourceName() == efaResource {
	...
	logger.Info("pod resources refreshed", "efa_mappings", len(result))
	return result
}
...
func NewAmazonEfaCollector(logger *slog.Logger) (Collector, error) {
	...
	descriptions := map[string]string{
		...
		"impaired_remote_conn_events":  "Number of EFA SRD connections entered an impaired state",
		...
		"rdma_read_wrs":                "Number of read wrs with RDMA",
		...
		"recv_bytes":                   "Number of recv bytes",
		"recv_wrs":                     "Number of recv wrs",
		...
		"retrans_timeout_events":       "Number of times SRD traffic reached timeout",
		"rx_bytes":                     "Number of bytes received",
		"rx_drops":                     "Number of packets dropped",
		"rx_pkts":                      "Number of packets received",
		"send_bytes":                   "Number of bytes sent",
		"send_wrs":                     "Number of wrs sent",
	...
	for metricName, description := range descriptions {
		i.metricDescs[metricName] = prometheus.NewDesc(
			...
			description,
			[]string{"device", "port", "pod", "namespace", "container"},
			nil,
		)
	}

	// Start the pod mapper background goroutine
	startPodMapper(logger)
...
func (c *AmazonEfaCollector) pushMetric(ch chan<- prometheus.Metric, name string, value uint64, deviceName string, port string, valueType prometheus.ValueType) {
	...
	ch <- prometheus.MustNewConstMetric(c.metricDescs[name], valueType, float64(value), deviceName, port, pod, ns, container)
}
...
func (c *AmazonEfaCollector) Update(ch chan<- prometheus.Metric) error {
	...
	for _, device := range devices {
		// Get pod info for info metric
		podMapMu.RLock()
		info, hasPod := podDevMap[device.Name]
		podMapMu.RUnlock()
		pod, ns, container := "", "", ""
		if hasPod {
			pod = info.Pod
		...
		infoDesc := prometheus.NewDesc(
			...
			[]string{"device", "pod", "namespace", "container"},
			nil,
		)
		ch <- prometheus.MustNewConstMetric(infoDesc, prometheus.GaugeValue, 1.0, device.Name, pod, ns, container)
